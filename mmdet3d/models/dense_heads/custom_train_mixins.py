...
class CustomAnchorTrainMixin(object):
    ...
    def anchor_target_3d(self,
        ...
        Args:
            ...
            custom_channels (int): The channel of custom attritubes.
            num_classes (int): The number of classes.
            num_custom (int): The number of classes of custom attritubes.
            sampling (bool): Whether to sample anchors.

        Returns:
            tuple (list, list, list, list, list, list, , list, list, int, int):
                Anchor targets, including labels, label weights,
                bbox targets, bbox weights, direction targets,
                direction weights, custom targets, custom weights, number of postive anchors and
        ...
        (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights,
         all_dir_targets, all_dir_weights, all_custom_targets, all_custom_weights, pos_inds_list,
        ...
        custom_targets_list = images_to_levels(all_custom_targets, num_level_anchors)
        custom_weights_list = images_to_levels(all_custom_weights, num_level_anchors)

        return (labels_list, label_weights_list, bbox_targets_list,
                bbox_weights_list, dir_targets_list, dir_weights_list, custom_targets_list, custom_weights_list,
    ...
    def anchor_target_3d_single(self,
        ...
        if isinstance(self.bbox_assigner,
            ...
            (total_labels, total_label_weights, total_bbox_targets,
             total_bbox_weights, total_dir_targets, total_dir_weights, total_custom_targets, total_custom_weights,
            ...
            for i, assigner in enumerate(self.bbox_assigner):
                ...
                if self.assign_per_class:
                    ...
                    anchor_targets = self.anchor_target_single_assigner(
                        ...
                        gt_bboxes_ignore, gt_labels[gt_per_cls], gt_custom[gt_per_cls], input_meta,
                        num_classes, sampling)
                else:
                    anchor_targets = self.anchor_target_single_assigner(
                        assigner, current_anchors, gt_bboxes, gt_bboxes_ignore,
                        gt_labels, gt_custom, input_meta, num_classes, sampling)

                (labels, label_weights, bbox_targets, bbox_weights,
                 dir_targets, dir_weights, custom_targets, custom_weights, pos_inds, neg_inds) = anchor_targets
                ...
                total_custom_targets.append(custom_targets.reshape(feat_size, 1, rot_angles))
                total_custom_weights.append(
                    custom_weights.reshape(feat_size, 1, rot_angles))
            ...
            total_custom_targets = torch.cat(total_custom_targets, dim=-2).reshape(-1)
            total_custom_weights = torch.cat(
                total_custom_weights, dim=-2).reshape(-1)
            ...
            return (total_labels, total_label_weights, total_bbox_targets,
                    ...
                    total_custom_targets, total_custom_weights, total_pos_inds, total_neg_inds)
            ...
            for i, assigner in enumerate(self.bbox_assigner):
                ...
                if self.assign_per_class:
                    ...
                    anchor_targets = self.anchor_target_single_assigner(
                        assigner, current_anchors, gt_bboxes[gt_per_cls, :],
                        gt_bboxes_ignore, gt_labels[gt_per_cls], gt_custom[gt_per_cls], input_meta,
                        num_classes, num_custom, sampling)
                else:
                    anchor_targets = self.anchor_target_single_assigner(
                        assigner, current_anchors, gt_bboxes, gt_bboxes_ignore,
                        gt_labels, gt_custom, input_meta, num_classes, num_custom, sampling)
                ...
                total_custom_targets.append(custom_targets)
                total_custom_weights.append(custom_weights)
                total_pos_inds.append(pos_inds)
                total_neg_inds.append(neg_inds)
            ...
            total_custom_targets = torch.cat(total_custom_targets, dim=0)
            total_custom_weights = torch.cat(total_custom_weights, dim=0)
            ...
            return self.anchor_target_single_assigner(self.bbox_assigner,
                                                      anchors, gt_bboxes,
                                                      gt_bboxes_ignore,
                                                      gt_labels, gt_custom, input_meta,
                                                      num_classes, num_custom, sampling)
    ...
    def anchor_target_single_assigner(self,
        ...
        Args:
            ...
            gt_bboxes_ignore (torch.Tensor): Ignored gt bboxes.
            gt_labels (torch.Tensor): Gt class labels.
            gt_custom (torch.Tensir:) Gt custom labels
            input_meta (dict): Meta info of each image.
            num_classes (int): The number of classes.
            num_custom (int): The number of classes of custom attribute.
            sampling (bool): Whether to sample anchors.
        ...
        custom_targets = anchors.new_zeros(num_valid_anchors, dtype=torch.float)
        custom_weights = anchors.new_zeros(num_valid_anchors, dtype=torch.float)
        if len(gt_bboxes) > 0:
        ...
        if gt_labels is not None:
            labels += num_classes
        if gt_custom is not None:
            custom_targets += num_custom
        ...
        if len(pos_inds) > 0:
            ...
            if gt_custom is None:
                custom_targets[pos_inds] = 1
            else:
                custom_targets[pos_inds] = gt_custom[
                    sampling_result.pos_assigned_gt_inds]

            if self.train_cfg.pos_weight <= 0:
                label_weights[pos_inds] = 1.0
                custom_weights[pos_inds] = 1.0
            else:
                label_weights[pos_inds] = self.train_cfg.pos_weight
                custom_weights[pos_inds] = self.train_cfg.pos_weight

        if len(neg_inds) > 0:
            label_weights[neg_inds] = 1.0
            custom_weights[neg_inds] = 1.0

        return (labels, label_weights, bbox_targets, bbox_weights, dir_targets,
                dir_weights, custom_targets, custom_weights, pos_inds, neg_inds)
